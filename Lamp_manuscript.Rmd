...
\subsection{Surveillance and Testing on College Campuses}
Numerous other college campuses have implemented
...
\section{Comparing Model Output to empirical
 data from
...
\section{Testing frequency, not testing sensitivity, is the major
driver of epidemic
...
df <- data.frame(day = seq(18, by = 7, length.out = 11), 
...
df %>% ggplot(aes(x = day, y = cumulative_MSU_cases)) + 
  ...
  labs(title = "Reported MSU COVID-19 Cases", caption = 'source: Gallatin county health department ')
...
\section{High testing frequency reduces epidemic size and
...
\section{The total number of missed cases is driven by prevalence
and testing
