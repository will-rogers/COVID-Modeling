...
```{r}
set.seed(12345)
output <- uni_sims_par(tst = c(0,250,500,1000,2000,4000), 
                       tst.timeline = c("Initial", "Sustained", "Both"),
                       compliance = c(0.75,1), # worst-best
                       init.prev = c(0,0.005,0.05), # best-mid-worst
                       ppn_sympt = c(0.2, 0.5), # best-worst
                       care.seeking = c(0.5,1), # worst-best
                       R0.on = c(2.5, 2.75, 3),  # best-mid-worst
                       R0.off = c(2.5, 2.75, 3), # best-mid-worst
                       test.scenario = c("No Delay","2 Days"),# best-worst
                       sens.pcr = 1, # for all intents this is reasonable
                       spec.pcr = 1, # for all intents this is reasonable
                       sens.lamp = c(0.85, 0.90, 0.95), # bad day/lab to good day/lab
                       spec.lamp = .99, # not dynamic yet
                       lamp.diagnostic = c(F,T), # will affect PCR demand
                       community.intro.daily.on = 1, 
                       community.prob.daily.on = c(0.1,.5, 1), # every ten, 2, or 1 days
                       community.intro.daily.off = 1, 
                       community.prob.daily.off = c(0.1,.5, 1), # every ten, 2, or 1 days
                       immunity = c(0.08358, 0.4179), # based on Fall 20 symptm, extrapolation to total
                       N0 = 16750, #campus pop
                       on.campus.prop = .25, #on/off division, only matters if we change on/off characteristics
                       contact.tracing.limit = c(0,50,100), # limit on number of contact traces per day
                       pooling = c(1,4,8), 
                       pooling.multi = c(1,.5,.25), #is the effect of pooling on accuracy 1:1, or do added pools only reduce sensitivity slightly?
