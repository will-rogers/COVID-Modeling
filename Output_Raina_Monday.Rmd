...
out$Testing <- factor(out$Testing, levels = c("No Surveillance","LAMP No Pool",
...
Based on 5% positivity in symptomatic testing
...
out %>% 
  ...
  geom_bar(stat="identity") +
  geom_errorbar(aes(ymax=upper, ymin=lower), color = "black") +
  labs(y="Proportion of Active Cases Discovered",
       x = "Testing Scenario") +
  theme_classic() + 
  facet_grid(.~Scenario) +
  guides(colour = guide_legend(override.aes = list(alpha = 1, size = 2)))+
...
out %>% 
  filter(!is.na(Testing) &
           ...
           days.to.isolate == 10) %>% 
  group_by(Testing,Scenario) %>%
  summarize(mean = mean(cum.iso.on),
            upper = quantile(cum.iso.on, 0.975),
            lower = quantile(cum.iso.on, 0.025)) %>%
  ggplot(aes(x = Testing, y = mean, color = Testing, fill = Testing)) +
  geom_bar(stat="identity") +
  geom_errorbar(aes(ymax=upper, ymin=lower), color = "black") +
  labs(y="Cumulative Number of On-Campus Isolation Rooms",
       x = "Testing Scenario") +
  theme_classic() + 
  facet_grid(.~Scenario) +
  guides(colour = guide_legend(override.aes = list(alpha = 1, size = 2)))+
...
out %>% 
  filter(!is.na(Testing)|
           compliance == 0.8&
           days.to.isolate == 10) %>% 
  ggplot(aes(x = day, y = isolation.complying.on, color = Testing, group = group)) +
  geom_line(alpha=0.15) +
  labs(y="Daily Number of On-Campus Isolation Rooms",
       x = "Day") +
  theme_classic() + 
  facet_grid(.~Scenario) +
  guides(colour = guide_legend(override.aes = list(alpha = 1, size = 2)))
...
p1 <- out %>% 
  ...
  summarize(mean = mean(cum.qua.on+cum.qua.off+cum.iso.off+cum.iso.on),
            upper = quantile(cum.qua.on+cum.qua.off+cum.iso.off+cum.iso.on, 0.975),
            lower = quantile(cum.qua.on+cum.qua.off+cum.iso.off+cum.iso.on, 0.025)) %>%
  mutate(mean = ifelse(Testing == "No Surveillance" & Iso.days == "7 Days", NA, mean),
         upper = ifelse(Testing == "No Surveillance" & Iso.days == "7 Days", NA, upper),
         lower = ifelse(Testing == "No Surveillance" & Iso.days == "7 Days", NA, lower)) %>% 
...
a %>%
  filter(Testing != "No Surveillance" & Iso.days == "10 Days") %>% 
