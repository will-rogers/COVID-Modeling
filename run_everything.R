...
set.seed(12345)
output <- uni_sims_par(tests=c(100, 250,500,1000,2000,3000,4000), compliance=1, introductions = 50, 
                       ppn_sympt = 0.2, care.seeking = 1, R0.on = 2.6, R0.off = 2.6, 
                       test.scenario = c("1 Day"), sens.pcr = c(1), spec.pcr = 1,
                       sens.lamp = c(.8,.85,.9,.95), spec.lamp = 1, lamp.diagnostic = c(F),
                       ncores=NULL)
...
#   filter(tests == 0) %>% 
#   group_by(day) %>% 
#   summarize(mean = mean(cum.all.symptomatics.on+cum.all.symptomatics.off),
#                      sd = sd(cum.all.symptomatics.on+cum.all.symptomatics.off),
#                      upper = mean + 1.96*sd,
#                      lower = mean - 1.96*sd) %>% 
