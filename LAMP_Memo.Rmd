...
best.cummulative <- best. %>% 
  ...
  summarize(cases.lower = quantile(cum.cases.on+cum.cases.off, 0.025),
            ...
            sym.lower = quantile(pcr.demand.sym, 0.025)/0.05,
            sym.higher = quantile(pcr.demand.sym, 0.975)/0.05,
            asym.lower = quantile(pcr.demand.asym, 0.025),
            asym.higher = quantile(pcr.demand.asym, 0.975)
            ) 

best. %>% 
  ggplot(aes(x = day, y = isolation.complying.on+isolation.complying.off,
             color = factor(tests))) +
  geom_line() +
...
df <- df %>% 
  mutate(student.days.lost.lower = iso.lower+quar.lower,
         ...
         cost.asym.lower = asym.lower*20,
         cost.asym.higher = asym.higher*20,
         cost.sym.lower = sym.lower*20,
         cost.sym.higher = sym.higher*20,
